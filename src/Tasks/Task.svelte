<script>
    ...
    onMount(() => { // função Lifecycle: acionada no início do componente.
        getTasks();
    });
    
    function getTasks() {
        fetch("https://todo-svelte3-4ddbd.firebaseio.com/tasks.json")
        .then(response => {
            if (!response.ok) {
    ...
    function updateTask() {
        ...
        fetch(`https://todo-svelte3-4ddbd.firebaseio.com/tasks/${id}.json`, {
            method: "PATCH",
            body: JSON.stringify(taskData),
            headers: { "Content-Type": "application/json" }
        })
